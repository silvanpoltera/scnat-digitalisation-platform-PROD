    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-10 gap-3">
          <div className="shrink-0 flex items-center gap-2 pr-3 border-r border-bd-faint">
          ...
          <button
            onClick={goPrev}
            className="shrink-0 p-1 rounded hover:bg-bg-elevated text-txt-tertiary hover:text-txt-secondary transition-colors"
          >
          ...
          <div className="flex-1 overflow-hidden relative min-w-0">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <span className="text-xs text-txt-secondary truncate">{item.text}</span>
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className="shrink-0 hidden sm:flex items-center gap-1 pl-3 border-l border-bd-faint">
            {tickerItems.map((_, i) => (
              <button
                key={i}
                onClick={() => setCurrent(i)}
                className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                  i === current ? "bg-scnat-red w-4" : "bg-bd-default hover:bg-bd-strong"
                }`}
              />
            ))}
          </div>
